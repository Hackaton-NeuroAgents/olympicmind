...
import { MapPin, Navigation, Clock, ShieldAlert, Footprints, TrainFront, Bus } from 'lucide-react';
          ...
          <div className="bg-surfaceHover/80 border border-accent/30 rounded-xl p-4 shadow-xl relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-3">
              <h3 className="font-bold text-white flex items-center gap-2">
                {getIcon(routeResult.routes[routeResult.recommended]?.name)}
                {routeResult.routes[routeResult.recommended]?.name}
              </h3>
              <div className="flex items-center gap-1 text-accent text-sm font-bold bg-accent/10 px-2 py-0.5 rounded">
                <Clock size={14} />
                {routeResult.routes[routeResult.recommended]?.time}
              </div>
            </div>
            ...
            <div className="space-y-2 relative">
              ...
              {routeResult.routes[routeResult.recommended]?.steps.map((step, idx) => (
